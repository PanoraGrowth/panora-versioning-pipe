...
@test "parse_version_components: period.major.minor" {
    source_config_parser "no-timestamp"
    parse_version_components "2.5.8"
    assert_equals "2" "$PARSED_PERIOD"
    assert_equals "5" "$PARSED_MAJOR"
    assert_equals "8" "$PARSED_MINOR"
}

@test "parse_version_components: major.minor only (period disabled)" {
    source_config_parser "with-v-prefix"
    parse_version_components "3.7"
    assert_equals "0" "$PARSED_PERIOD" "period should default to 0 when disabled"
...
@test "parse_version_components: large numbers" {
    ...
    assert_equals "10" "$PARSED_PERIOD"
